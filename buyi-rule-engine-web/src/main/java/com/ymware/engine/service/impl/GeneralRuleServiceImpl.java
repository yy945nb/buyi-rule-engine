...
import com.ymware.engine.common.util.VersionUtils;
...
import com.ymware.engine.domain.rule.service.Rule;
import com.ymware.engine.domain.value.model.VariableType;
import com.ymware.engine.config.Context;
import com.ymware.engine.common.enums.DataStatus;
...
import com.ymware.engine.listener.event.GeneralRuleEvent;
import com.ymware.engine.service.WebValueResolve;
import com.ymware.engine.entity.*;
import com.ymware.engine.mapper.RuleEngineGeneralRuleMapper;
...
public class GeneralRuleServiceImpl implements GeneralRuleService {
    ...
    public PageResult<ListGeneralRuleResponse> list(PageRequest<ListGeneralRuleRequest> pageRequest) {
        ...
        Set<Long> userIds = records.stream().map(RuleEngineGeneralRule::getCreateUserId).collect(Collectors.toSet());
        Map<Long, RuleEngineUser> ruleEngineUserMap = this.userService.getMapByUserIds(userIds);
        List<ListGeneralRuleResponse> collect = records.stream().map(m -> {
            ...
            Long createUserId = m.getCreateUserId();
     ...
     */
    @Override
    public GeneralRuleDefinition getRuleDefinition(Long id) {
     ...
     */
    @Override
    public GetGeneralRuleResponse getRuleConfig(Long id) {
        RuleEngineGeneralRule ruleEngineGeneralRule = this.ruleEngineGeneralRuleManager.getById(id);
        if (ruleEngineGeneralRule == null) {
            throw new ApiException(ErrorCodeEnum.RULE9999404.getCode(), "不存在规则:{}", id);
        }
     ...
     */
    @Override
    public Long saveRuleDefinition(GeneralRuleDefinition ruleDefinition) {
        // 创建规则
    ...
    public Boolean saveDefaultAction(SaveDefaultActionRequest saveActionRequest) {
        Long generalRuleId = saveActionRequest.getGeneralRuleId();
    ...
    public Boolean goBack(GoBackRequest goBackRequest) {
        Long dataId = goBackRequest.getDataId();
     ...
     */
    @Override
    public Boolean deleteHistoricalRules(Long id) {
