...
import com.ymware.engine.store.manager.*;


import cn.hutool.core.collection.CollUtil;
import com.ymware.engine.condition.Condition;
import com.ymware.engine.condition.ConditionGroup;
...
import com.ymware.engine.entity.*;
import com.ymware.engine.store.manager.RuleEngineConditionGroupConditionManager;
import com.ymware.engine.store.manager.RuleEngineConditionGroupManager;
import com.ymware.engine.store.manager.RuleEngineConditionManager;
import com.ymware.engine.vo.common.RearrangeRequest;
import com.ymware.engine.vo.condition.ConditionBody;
import com.ymware.engine.vo.condition.ConditionGroupCondition;
import com.ymware.engine.vo.condition.ConditionGroupConfig;
import com.ymware.engine.vo.condition.ConfigBean;
import com.ymware.engine.vo.condition.group.SaveOrUpdateConditionGroup;
...
public class ConditionGroupServiceImpl implements ConditionGroupService {
    ...
    @Resource
    private WebValueResolve valueResolve;
     ...
     */
    @Override
    public Long saveOrUpdateConditionGroup(SaveOrUpdateConditionGroup saveOrUpdateConditionGroup) {
     ...
     */
    @Override
    public void saveConditionGroup(Long ruleId, List<ConditionGroupConfig> conditionGroup) {
    ...
    public Boolean delete(Long id) {
        ...
        if (CollUtil.isNotEmpty(list)) {
            ...
            this.ruleEngineConditionGroupConditionManager.lambdaUpdate()
                    ...
                    .remove();
            // 再去根据条件查询引用的条件
            List<Long> collect = list.stream().map(RuleEngineConditionGroupCondition::getConditionId).collect(Collectors.toList());
    ...
    @Async
    @Override
    public void removeConditionGroupByRuleIds(List<Long> ruleIds) {
        ...
        if (CollUtil.isNotEmpty(engineConditionGroups)) {
            List<Long> engineConditionGroupIds = engineConditionGroups.stream().map(RuleEngineConditionGroup::getId).collect(Collectors.toList());
     ...
     */
    @Override
    public List<ConditionGroupConfig> getConditionGroupConfig(Long ruleId) {
        ...
        // 加载所有的用到的条件组条件
        Set<Long> conditionGroupIds = engineConditionGroups.stream().map(RuleEngineConditionGroup::getId).collect(Collectors.toSet());
        List<RuleEngineConditionGroupCondition> ruleEngineConditionGroupConditions = this.ruleEngineConditionGroupConditionManager.lambdaQuery()
                ...
                .list();
        Map<Long, List<RuleEngineConditionGroupCondition>> conditionGroupConditionMaps;
        Map<Long, RuleEngineCondition> conditionMap = Collections.emptyMap();
        Map<Long, RuleEngineInputParameter> inputParameterMap = Collections.emptyMap();
        Map<Long, RuleEngineVariable> variableMap = Collections.emptyMap();
        if (CollUtil.isNotEmpty(ruleEngineConditionGroupConditions)) {
            ...
            Set<Long> conditionIds = conditionGroupConditionMaps.values().stream().flatMap(Collection::stream)
