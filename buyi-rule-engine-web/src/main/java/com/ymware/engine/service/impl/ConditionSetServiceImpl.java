...
import com.ymware.engine.service.ConditionSetService;
import com.ymware.engine.service.WebValueResolve;
import com.ymware.engine.entity.RuleEngineCondition;
import com.ymware.engine.entity.RuleEngineConditionGroup;
...
public class ConditionSetServiceImpl implements ConditionSetService {
     ...
     */
    @Override
    public ConditionSet loadConditionSet(Long ruleId) {
        ...
        // 加载所有的用到的条件组条件
        Set<Long> conditionGroupIds = conditionGroups.stream().map(RuleEngineConditionGroup::getId).collect(Collectors.toSet());
        Map<Long, List<RuleEngineConditionGroupCondition>> conditionGroupConditionMaps = this.ruleEngineConditionGroupConditionManager.lambdaQuery()
        ...
        Set<Long> conditionIds = conditionGroupConditionMaps.values().stream().flatMap(Collection::stream).map(RuleEngineConditionGroupCondition::getConditionId).collect(Collectors.toSet());
        ...
        Map<Long, RuleEngineCondition> conditionMap = ruleEngineConditions.stream().collect(Collectors.toMap(RuleEngineCondition::getId, Function.identity()));
        ConditionSet conditionSet = new ConditionSet();
