...
import com.ymware.engine.mapper.RuleEngineDataReferenceMapper;
import com.ymware.engine.vo.reference.ReferenceData;
import com.ymware.engine.vo.reference.ReferenceDataMap;
import com.ymware.engine.vo.rule.general.DefaultAction;
...
public class DataReferenceServiceImpl implements DataReferenceService {
     ...
     */
    @Override
    public void validDataReference(Integer type, Long refDataId) {
    ...
    public void resolve(ReferenceData referenceData, ConfigValue configValue) {
        ...
        if (VariableType.INPUT_PARAMETER.getType().equals(type)) {
            referenceData.addInputParameterId(Long.valueOf(value));
        } else if (VariableType.VARIABLE.getType().equals(type)) {
            referenceData.addVariableId(Long.valueOf(value));
        } else if (VariableType.GENERAL_RULE.getType().equals(type)) {
            referenceData.addGeneralRuleId(Long.valueOf(value));
        }
    }
     ...
     */
    @Override
    public void updateToDevStatus(Integer dataType, Long dataId) {
    ...
    private ReferenceData getReferenceData(Integer dataType, Long dataId, String version) {
        RuleEngineDataReference dataReference = this.ruleEngineDataReferenceManager.lambdaQuery()
     ...
     */
    @Override
    public ReferenceDataMap getReferenceDataMap(Integer dataType, Long dataId, String version) {
        ...
        Set<Long> variableIds = referenceData.getVariableIds();
        ...
        Set<Long> generalRuleIds = referenceData.getGeneralRuleIds();
    ...
    public Collection<RuleParameter> getRuleSetParameters(Long id, String version) {
        ...
        Set<Long> dataInputParameterIds = referenceData.getInputParameterIds();
        Set<Long> variableIds = referenceData.getVariableIds();
        Set<Long> generalRuleIds = referenceData.getGeneralRuleIds();
        // 寻找基本规则中的引用
        if (CollUtil.isNotEmpty(generalRuleIds)) {
            ...
            for (RuleEngineGeneralRulePublish generalRulePublish : generalRulePublishs) {
                ...
                if (rd != null) {
                    ...
                    Set<Long> rdVariableIds = rd.getVariableIds();
