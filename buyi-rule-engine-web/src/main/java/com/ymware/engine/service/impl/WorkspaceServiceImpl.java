...
import com.ymware.engine.mapper.RuleEngineWorkspaceMapper;
...
public class WorkspaceServiceImpl implements WorkspaceService {
    ...
    public PageResult<ListWorkspaceResponse> list(PageRequest<ListWorkspaceRequest> pageRequest) {
        ...
        Long total = this.ruleEngineWorkspaceMapper.totalWorkspace(userId, query, page);
    ...
    public List<ListWorkspaceResponse> convertRuleEngineWorkspaceList(List<RuleEngineWorkspace> ruleEngineWorkspaces) {
        ...
        List<Long> workspaceIds = listWorkspaceResponses.stream().map(Workspace::getId).collect(Collectors.toList());
        ...
        // 工作空间管理映射表
        Map<Long, List<RuleEngineUserWorkspace>> workspaceAdminMap = workspaceList.stream().collect(Collectors.groupingBy(RuleEngineUserWorkspace::getWorkspaceId));
        // 用户映射表
        Set<Long> adminUserIds = workspaceList.stream().map(RuleEngineUserWorkspace::getUserId).collect(Collectors.toSet());
        Map<Long, RuleEngineUser> userMap = ruleEngineUserManager.lambdaQuery().in(RuleEngineUser::getId, adminUserIds).list().stream().collect(Collectors.toMap(RuleEngineUser::getId, Function.identity()));
     ...
     */
    @Override
    public boolean hasWorkspacePermission(Long workspaceId, Long userId) {
     ...
     */
    @Override
    public boolean isWorkspaceAdministrator(Long userId, Long workspaceId) {
     ...
     */
    @Override
    public Workspace getFirstWorkspace(Long userId, boolean isAdmin) {
        ...
        if (ruleEngineWorkspace == null) {
            // 没有可用工作空间
            throw new ApiException(ErrorCodeEnum.RULE4010);
        }
        Workspace workspace = new Workspace();
        workspace.setId(ruleEngineWorkspace.getId());
        workspace.setName(ruleEngineWorkspace.getName());
    ...
    @Async
    @Transactional(rollbackFor = Exception.class)
    public void syncDeleteWorkspaceData(Long id) {
        // 普通规则
